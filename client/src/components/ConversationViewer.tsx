...
export default function ConversationViewer({
  ...
  const contactTags = trpc.sales.getContactTags.useQuery({
    contactId: conversation.contactId,
  });
  const pipelines = trpc.sales.listPipelines.useQuery();
  const createTag = trpc.sales.createTag.useMutation({
    onSuccess: () => {
      setNewTagName("");
      utils.sales.listTags.invalidate();
    },
  });
  const toggleTag = trpc.sales.toggleContactTag.useMutation({
    onSuccess: () => utils.sales.getContactTags.invalidate(),
  });
  const createDeal = trpc.sales.createDeal.useMutation({
    onSuccess: () => {
      setShowDealForm(false);
      setDealTitle("");
      utils.sales.getBoard.invalidate();
    },
  });
  const deleteMessages = trpc.messages.deleteMessages.useMutation({
    onSuccess: async () => {
  ...
  const deleteSelectedMessages = () => {
    ...
    if (
      !window.confirm(
        `Excluir permanentemente ${messageIds.length} mensagem(ns) do CRM?`
      )
    )
      return;
    deleteMessages.mutate({ conversationId, messageIds });
  };
    ...
    <Card className="border-0 shadow-sm h-full flex flex-col">
      <CardHeader className="border-b space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              setDealTitle(conversation.contact?.name || "Nova oportunidade");
            }}
          >
            <Kanban className="mr-1 h-4 w-4" /> Criar card
          </Button>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {allTags.data?.map((tag: any) => {
            const selected =
              contactTags.data?.some((item: any) => item.id === tag.id) ||
              false;
            return (
              <button
                key={tag.id}
                ...
                className={`rounded-full border px-2.5 py-1 text-xs font-medium ${selected ? "text-white" : "bg-white text-slate-600"}`}
                style={
                  selected
          ...
          <Input
            ...
            onKeyDown={event => {
              if (event.key === "Enter" && newTagName.trim())
                createTag.mutate({ name: newTagName, color: "#2563eb" });
            }}
          />
          <Button
            size="sm"
        ...
        {showDealForm && (
          <div className="grid gap-2 rounded-xl border border-emerald-200 bg-emerald-50 p-3 sm:grid-cols-3">
            ...
            >
              ...
              {pipelines.data
                ?.find(pipeline => pipeline.id === Number(pipelineId))
            ...
            <div className="sm:col-span-3 flex gap-2">
              <Button
                size="sm"
                disabled={!pipelineId || !stageId || !dealTitle.trim()}
                onClick={() =>
                  createDeal.mutate({
      ...
      <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
        {messageList.length > 0 && (
          <div className="sticky top-0 z-10 flex flex-wrap gap-2 rounded-lg border bg-white/95 p-2 shadow-sm backdrop-blur">
            <Button
              size="sm"
              variant="outline"
              onClick={() =>
                setSelectedMessageIds(
                  new Set(messageList.map((message: any) => message.id))
                )
              }
            >
            ...
            <Button
              ...
              disabled={!selectedMessageIds.size || deleteMessages.isPending}
            >
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <Button
                ...
                onClick={() =>
                  setMessageContent(getSuggestion.data?.suggestion || "")
                }
              >
                Usar sugestão
              </Button>
