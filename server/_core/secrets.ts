...
function encryptionKey(version?: string): Buffer {
  ...
  if (configured && version !== "v1d") {
    ...
    if (decoded.length !== 32) throw new Error("SECRETS_ENCRYPTION_KEY deve ter 32 bytes em base64");
    return decoded;
  }
  const jwt = process.env.JWT_SECRET;
  if (!jwt) throw new Error("JWT_SECRET não configurada para proteger credenciais");
  return crypto.createHash("sha256").update(`credential-encryption:${jwt}`).digest();
}
...
export function decryptSecret(payload: string): string {
  ...
  if (!["v1", "v1d"].includes(version) || !ivValue || !tagValue || !encryptedValue) throw new Error("Segredo criptografado inválido");
  const decipher = crypto.createDecipheriv(ALGORITHM, encryptionKey(version), Buffer.from(ivValue, "base64"));
